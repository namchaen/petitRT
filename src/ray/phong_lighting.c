...
/*   Updated: 2022/11/11 18:03:03 by chaejkim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	is_in_shadow(const t_scene *scene, const t_light *light, t_vec3 light_dir)//
{
	float	light_len;
	t_vec3	v_tmp;
	t_ray	light_ray;
...
static t_color3	point_light_get(const t_scene *scene, const t_light *light)
{
	...
	light_dir = vsub_(light->orig, scene->rec.p);
	if (is_in_shadow(scene, light, light_dir))
